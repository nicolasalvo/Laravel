import * as THREE from 'three'
import { ref } from 'vue'
import { createPlayer } from '../Player'
import { createObstacle } from '../Obstacle'
import { createCoin } from '../Coins'
import { createHeart } from '../Heart'

export function useThreeGame(canvasRef) {
  // Estado para Vue (HUD)
  const score = ref(0)
  const best = ref(Number(localStorage.getItem('bestScore') ?? 0))
  const speed = ref(7) // unidades/segundo
  const isGameOver = ref(false)
  const lives = ref(1)

  // Three.js
  let scene, camera, renderer
  let animationId = null
  let clock = null
  // Entidades
  let player = null
  const obstacles = []
  const hearts = []

  // Suelo/pista
  let ground = null

  // Input
  const keys = new Set()

  // Timers de spawn
  let obstacleTimer = 0
  let coinTimer = 0
  let heartTimer = 0
  const coins = []

  // Config juego
  const LANES = [-2, 0, 2]
  const SPAWN_Z = -35
  const DESPAWN_Z = 10
  const PLAYER_Z = 0

  // Física simple salto
  const GRAVITY = -20
  const JUMP_V = 9.5

  // Colisiones
  const playerBox = new THREE.Box3()
  const tempBox = new THREE.Box3()

  function initThree() {
    scene = new THREE.Scene()
    scene.background = new THREE.Color(0x101018)
    scene.fog = new THREE.Fog(0x101018, 12, 55)

    const w = window.innerWidth
    const h = window.innerHeight

    // Suelo / pista básico (sin modelo 3D)
    const groundGeo = new THREE.BoxGeometry(10, 1, 100)
    const groundMat = new THREE.MeshStandardMaterial({
      color: 0x222222,
      roughness: 0.8
    })

    // Asignamos a la variable externa 'ground'
    ground = new THREE.Mesh(groundGeo, groundMat)
    ground.position.set(0, -0.5, -20)
    ground.receiveShadow = true
    scene.add(ground)

    // Líneas laterales (visual - Rails)
    const railGeo = new THREE.BoxGeometry(0.15, 0.4, 100)
    const railMat = new THREE.MeshStandardMaterial({ color: 0x5a5aff, roughness: 0.6 })
    const leftRail = new THREE.Mesh(railGeo, railMat)
    const rightRail = new THREE.Mesh(railGeo, railMat)
    leftRail.position.set(-4, 0.2, -20)
    rightRail.position.set(4, 0.2, -20)
    leftRail.receiveShadow = true
    rightRail.receiveShadow = true
    scene.add(leftRail, rightRail)

    camera = new THREE.PerspectiveCamera(70, w / h, 0.1, 200)
    camera.position.set(0, 4.2, 8)
    camera.lookAt(0, 1, -6)

    renderer = new THREE.WebGLRenderer({
      canvas: canvasRef.value,
      antialias: true
    })
    renderer.setSize(w, h)
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
    renderer.shadowMap.enabled = true

    // Luces
    scene.add(new THREE.AmbientLight(0xffffff, 0.55))

    const dir = new THREE.DirectionalLight(0xffffff, 1.1)
    dir.position.set(6, 10, 6)
    dir.castShadow = true
    dir.shadow.mapSize.width = 1024
    dir.shadow.mapSize.height = 1024
    scene.add(dir)

    // Jugador
    player = createPlayer()
    player.castShadow = true
    player.position.z = PLAYER_Z
    scene.add(player)

    clock = new THREE.Clock()

    // Eventos
    window.addEventListener('resize', onResize)
    window.addEventListener('keydown', onKeyDown)
    window.addEventListener('keyup', onKeyUp)
  }

  function onResize() {
    if (!renderer || !camera) return
    const w = window.innerWidth
    const h = window.innerHeight
    camera.aspect = w / h
    camera.updateProjectionMatrix()
    renderer.setSize(w, h)
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2))
  }

  function onKeyDown(e) {
    keys.add(e.code)

    if (e.code === 'ArrowLeft' || e.code === 'KeyA') moveLane(-1)
    if (e.code === 'ArrowRight' || e.code === 'KeyD') moveLane(1)
    if (e.code === 'Space') jump()

  }

  function onKeyUp(e) {
    keys.delete(e.code)
  }

  function onCoinCollected() {
    score.value += 10
  }

  function onHeartCollected() {
    lives.value += 1
  }

  function moveLane(dir) {
    if (!player || isGameOver.value) return
    const currentX = player.userData.targetX
    const laneIndex = LANES.indexOf(currentX)
    const nextIndex = THREE.MathUtils.clamp(laneIndex + dir, 0, LANES.length - 1)
    player.userData.targetX = LANES[nextIndex]
  }

  function jump() {
    if (!player || isGameOver.value) return
    if (player.userData.isJumping) return
    player.userData.isJumping = true
    player.userData.vy = JUMP_V
  }

  function spawnObstacle() {
    const o = createObstacle()
    o.castShadow = true
    o.position.z = SPAWN_Z
    o.position.x = LANES[Math.floor(Math.random() * LANES.length)]
    o.position.y = 0.45
    obstacles.push(o)
    scene.add(o)
  }

  function spawnCoin() {
    let coin = createCoin()
    coin.castShadow = true
    coin.position.z = SPAWN_Z
    coin.position.x = LANES[Math.floor(Math.random() * LANES.length)]
    coin.position.y = 0.45
    coins.push(coin)
    scene.add(coin)
  }

  function spawnHeart() {
    let heart = createHeart()
    heart.castShadow = true
    heart.position.z = SPAWN_Z
    heart.position.x = LANES[Math.floor(Math.random() * LANES.length)]
    heart.position.y = 0.45
    hearts.push(heart)
    scene.add(heart)
  }

  function onHeartCollected() {
    lives.value++
  }

  // Al tocar una moneda
  function onCoinCollected() {
    score.value += 10
  }

  function checkCoinCollisions() {
    for (let i = coins.length - 1; i >= 0; i--) {
      const coin = coins[i]
      if (coin.userData.isCollected) continue
      const coinBox = new THREE.Box3().setFromObject(coin)
      if (playerBox.intersectsBox(coinBox)) {
        coin.userData.isCollected = true
        scene.remove(coin)
        coins.splice(i, 1)
        onCoinCollected()
      }
    }
  }

  function checkHeartCollisions() {
    for (let i = hearts.length - 1; i >= 0; i--) {
      const heart = hearts[i]
      if (heart.userData.isCollected) continue
      const heartBox = new THREE.Box3().setFromObject(heart)
      if (playerBox.intersectsBox(heartBox)) {
        heart.userData.isCollected = true
        scene.remove(heart)
        hearts.splice(i, 1)
        onHeartCollected()
      }
    }
  }

  function updatePlayer(dt) {
    // Suavizado lateral hacia el carril objetivo
    const targetX = player.userData.targetX
    player.position.x = THREE.MathUtils.damp(player.position.x, targetX, 12, dt)

    // Salto simple
    if (player.userData.isJumping) {
      player.userData.vy += GRAVITY * dt
      player.position.y += player.userData.vy * dt

      if (player.position.y <= player.userData.groundY) {
        player.position.y = player.userData.groundY
        player.userData.isJumping = false
        player.userData.vy = 0
      }
    }
  }

  function updateWorld(dt) {
    // Aumenta dificultad progresiva
    speed.value = Math.min(1000, speed.value + dt * 1.5)

    // Spawn con timers (ajusta la frecuencia)
    obstacleTimer += dt
    coinTimer += dt

    if (speed.value > 40) {
      if (obstacleTimer >= 0.2) {
        obstacleTimer = 0
        spawnObstacle()
      }
    } else if (speed.value > 20) {
      if (obstacleTimer >= 0.4) {
        obstacleTimer = 0
        spawnObstacle()
      }
    } else if (speed.value > 10) {
      if (obstacleTimer >= 0.6) {
        obstacleTimer = 0
        spawnObstacle()
      }
    } else {
      if (obstacleTimer >= 1) {
        obstacleTimer = 0
        spawnObstacle()
      }
    }

    // Mover obstáculos/monedas hacia el jugador
    const dz = speed.value * dt

    heartTimer += dt

    if (heartTimer >= 15) {
      heartTimer = 0
      spawnHeart()
    }

    if (coinTimer >= 1.5) {
      coinTimer = 0
      spawnCoin()
    }

    // Al tocar una moneda 

    for (let i = obstacles.length - 1; i >= 0; i--) {
      const o = obstacles[i]
      o.position.z += dz
      if (o.position.z > DESPAWN_Z) {
        scene.remove(o)
        obstacles.splice(i, 1)
      }
    }

    for (let i = coins.length - 1; i >= 0; i--) {
      const c = coins[i]
      c.position.z += dz
      if (c.position.z > DESPAWN_Z) {
        scene.remove(c)
        coins.splice(i, 1)
      }
    }

    for (let i = hearts.length - 1; i >= 0; i--) {
      const h = hearts[i]
      h.position.z += dz
      if (h.position.z > DESPAWN_Z) {
        scene.remove(h)
        hearts.splice(i, 1)
      }
    }

    // Infinite Floor Movement
    const speedZ = speed.value * dt;
    if (ground) {
      ground.children.forEach(tile => {
        tile.position.z += speedZ;
        // Si sale de la cámara (ej > 20), lo mandamos al fondo
        if (tile.position.z > 20) {
          tile.position.z -= 90;
        }
      });
    }
  }

  function checkCollisions() {
    // Caja del jugador
    playerBox.setFromObject(player)

    // Obstáculos
    for (let i = 0; i < obstacles.length; i++) {
      tempBox.setFromObject(obstacles[i])
      if (playerBox.intersectsBox(tempBox)) {
        lives.value--
        scene.remove(obstacles[i])
        obstacles.splice(i, 1)

        if (lives.value <= 0) {
          gameOver()
        }
        return
      }
    }
  }

  function gameOver() {
    isGameOver.value = true
    if (score.value > best.value) {
      best.value = score.value
      localStorage.setItem('bestScore', String(best.value))
    }
  }

  function updateScore(dt) {
    // Score por tiempo (además de monedas)
    score.value += Math.floor(dt * 10)
  }

  function animate() {
    animationId = requestAnimationFrame(animate)

    const dt = clock.getDelta()

    if (!isGameOver.value) {
      updatePlayer(dt)
      updateWorld(dt)
      checkCollisions()
      checkCoinCollisions()
      checkHeartCollisions()
      updateScore(dt)
    }

    renderer.render(scene, camera)
  }

  function clearEntities() {
    for (const o of obstacles) scene.remove(o)
    obstacles.length = 0
  }

  function resetGame() {
    score.value = 0
    speed.value = 7
    isGameOver.value = false

    speed.value = 7
    lives.value = 1
    isGameOver.value = false

    clearEntities()
    for (const h of hearts) scene.remove(h)
    hearts.length = 0

    // Reset jugador
    player.position.set(0, 0.45, PLAYER_Z)
    player.userData.targetX = 0
    player.userData.isJumping = false
    player.userData.vy = 0

    // Reset suelo
    ground.position.z = -20

    obstacleTimer = 0
  }

  function start() {
    initThree()
    resetGame()
    animate()
  }

  function stop() {
    if (animationId) cancelAnimationFrame(animationId)
    animationId = null

    window.removeEventListener('resize', onResize)
    window.removeEventListener('keydown', onKeyDown)
    window.removeEventListener('keyup', onKeyUp)

    // Limpieza básica
    if (renderer) {
      renderer.dispose()
      renderer = null
    }
    scene = null
    camera = null
  }

  function restart() {
    resetGame()
  }

  return {
    start,
    stop,
    restart,
    score,
    best,
    speed,
    isGameOver,
    lives,
  }
}